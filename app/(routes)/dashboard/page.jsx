...
export default function DashboardPage() {
  ...
  const [itemsPerPage] = useState(5); // Number of categories per page
  ...
  // Pagination calculations for expense details
  const totalPages = Math.ceil(dashboardData.categories.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentCategories = dashboardData.categories.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8">
      ...
      <Tabs defaultValue="categories" className="space-y-4">
        ...
        {/* Detailed Breakdown of Expenses with Pagination */}
        <TabsContent value="details" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Expense Details</CardTitle>
              ...
              <div className="flex justify-between mt-4">
                ...
                <div>
                  {getPageNumbers().map((number) => (
                    <Button key={number} onClick={() => handlePageClick(number)} className={currentPage === number ? 'bg-blue-500 text-white' : ''}>
                      {number}
                    </Button>
                  ))}
                </div>
